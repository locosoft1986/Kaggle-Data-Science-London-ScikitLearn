...
knn_best_pred = pd.DataFrame([0] + knn_best.predict(x_test), columns=['Solution'])
rf_best_pred = pd.DataFrame([0] + rf_best.predict(x_test), columns=['Solution'])
#voting_clf_pred = pd.DataFrame(clf.predict(x_test))

knn_best_pred.index += 1
knn_best_pred.index.name = 'Id'
